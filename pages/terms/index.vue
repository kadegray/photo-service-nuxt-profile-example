<template>
  <div>
    ...
    <div v-if="loading" class="text-center py-12 text-gray-400">Loading tags...</div>
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <p v-if="term.taxonomy" class="text-xs text-gray-500">{{ term.taxonomy.name }}</p>
            </div>
            <span class="text-sm text-gray-400">{{ term.photo_count }} photos</span>
          </div>
          <p v-if="term.description" class="text-sm text-gray-400 mt-2">{{ term.description }}</p>
        </NuxtLink>
      </div>
...
const { terms, loading, error, pagination, nextPage, prevPage } = useTerms()
